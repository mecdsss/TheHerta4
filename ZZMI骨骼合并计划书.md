...
**游戏内 dump 实证记录续（08-23-110955，Present 时序版）**：11 个部件全部换绑生效（vs-t0 无 dump）；SO 重建残差 sub-mm（19086112 max=0.0000、454ff522 max=0.0015、48625d6d max=0.0013，数据链自洽）。**cb1 捕获脆弱性实锤**：同一帧里 16B 垃圾小块（draw 313/319/315-318/196）、2048/4096B 共享变换数组（230/231/236/237/233）、两套并存的对象变换（(0,0,0) 原点与 (-13.9,1.9,-9.6) 世界位）同时存在，且代表部件的"帧内最后一个渲染 draw"内容随场景变化（122152 里 313 是合法块、110955 里是 16B 垃圾）——last-wins 捕获可能拿到垃圾/错误位置。修复：CS 形态校验（row3.w=1 + 旋转行单位 + 平移量级）+ **无效帧保持上一次有效校准内容不覆写**（抗逐帧交替闪跳；仅启动期全零槽位直拷兜底）；palette 副本资源显式 `type=Buffer stride=48`（空声明 SRV 视图不受控会读垃圾矩阵——疑为"一针卡一针"主因）；palette copy 补 `unless_null`。
