...
**游戏内 dump 实证记录续（08-23-110955，Present 时序版）**：11 个部件全部换绑生效（vs-t0 无 dump）；SO 重建残差 sub-mm（19086112 max=0.0000、454ff522 max=0.0015、48625d6d max=0.0013，数据链自洽）。**cb1 捕获脆弱性实锤**：同一帧里 16B 垃圾小块（draw 313/319/315-318/196）、2048/4096B 共享变换数组（230/231/236/237/233）、两套并存的对象变换（(0,0,0) 原点与 (-13.9,1.9,-9.6) 世界位）同时存在，且代表部件的"帧内最后一个渲染 draw"内容随场景变化（122152 里 313 是合法块、110955 里是 16B 垃圾）——last-wins 捕获可能拿到垃圾/错误位置。修复链：①palette 副本资源显式 `type=Buffer stride=48`（空声明 SRV 视图不受控会读垃圾矩阵逐帧乱跳——疑为"一针卡一针"主因）；②palette copy 补 `unless_null`；③CS 兜底策略="宁可退化不错校准"：形态无效（16B 垃圾/未捕获）或**平移差 >2m**（共享数组 0 号记录/别的实例错误内容；同角色合法组间差 <1m）一律直拷（跟随源件运动、常量空间差、不闪跳），另有**校准总开关 `$zz_ms_calibrate`**（Constants 里 0/1，A/B 隔离验证用）。（注：曾短暂试过"无效帧保持上一次内容"，实机表现为错误内容被持续锁住（"顶得更陡"），已回退为直拷。）
