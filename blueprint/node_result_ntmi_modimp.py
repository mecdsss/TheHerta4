    ...
    def execute(self, context):
        ...
        try:
            ...
            if tree is None or node is None:
                self.report({"ERROR"}, "当前蓝图中未找到 NTMI ModImp 输出节点。")
                return {"CANCELLED"}

            BlueprintExportHelper.set_runtime_blueprint_tree(tree)
            execute_ntmi_modimp_export(context=context, tree=tree, node=node)

            self.report({"INFO"}, "NTMI ModImp 导出成功。")
...
class SSMTNode_Result_Output_NTMIModImp(SSMTNodeBase):
    ...
    def draw_buttons(self, context, layout):
        ...
        if self.use_custom_export_dir:
            ...
            output_box.label(text="默认：使用当前 SSMT 导出目录", icon="INFO")
        ...
        try:
            ...
            check_operator = dep_box.operator(
                "ssmt.check_ntmi_modimp_dependency",
                text="检测前置插件",
                icon="VIEWZOOM",
            )
